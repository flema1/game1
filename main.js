...
//note code from this git was not references only the game rules qoutes below 
    /*2x Reverse
...
if (itsXsturn="computer"){
    computerStratategyAI(computerHand);
}
// else{
     whoGoes=turnTracker(itsXsturn);// reassigning global variable for turn tracker
 ...
 //need function to check if player has "uno" cards left
    ...
    let moveCardstoDiscardPile=function(hand, key){
       ...
       for (let card=0; card<hand.length; card++){
           ...
           removeCard(cardRemoved[0].id); 
               }
     
              // console.log ("bobo");
              console.log("card moved to discard pile");
...
let removeCard=function(id){
    let $compHand=$('#one_display .unoCard'); 
    console.log ($compHand); 
     console.log (id); 
    console.log ("card ids in the comp hand are:"); 
$compHand.each(function( index ) {
...
let computerStratategyAI=function(computerHand){
   ...
   if (computerStats.numberMatches>computerStats.colorMatches){
       ...
       considerSpecialCards( computerStats);//what if current action cards were played? 
          ...
          console.log("no action cards detected. Draw Card");
  ...
  let temp1=[]; 
    temp1.push(card); 
displayCards(temp1, "#one_display")
 //temp1=[]; 
